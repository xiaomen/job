...
from paginator import gen_list_page_obj
from models import *
...
def get_all_jobs(page, **kw):
    page = int(page)
    page_obj = Article.get_query_page_without_intern(
        page, per_page=PAGE_NUM, **kw)
...
def get_favorite_job_by_user(uid, page):
    page = int(page)
    page_obj = Favorite.query.filter_by(
        uid=uid).paginate(page, per_page=PAGE_NUM)
